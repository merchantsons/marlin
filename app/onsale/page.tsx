...
const Onsale: React.FC = () => {
  ...
  useEffect(() => {
    const query = `*[_type == "shopco" && discount != null] | order(_createdAt desc) {
      _id,
      id,
      title,
      type,
  ...
  const sortProducts = (option: string) => {
    ...
    switch (option) {
      ...
      case 'Low to High':
        ...
        break;
      case 'High to Low':
        sorted = sorted.sort((a, b) => parseFloat(b.discPrice || b.price) - parseFloat(a.discPrice || b.price));
        break;
      default:
        break;
    }
      ...
      <div className="w-full lg:max-w-7xl justify-self-center">
        <div className="flex justify-between items-center mt-4 mx-6">
          ...
          <div className="text-[1.4vh] text-gray-700">
            ...
            <span className="font-bold text-blue-950">{sortedProducts.filter(product => parseInt(product.qty) > 0).length}</span>  (On Sale Products Available)
          </div>
        </div>
